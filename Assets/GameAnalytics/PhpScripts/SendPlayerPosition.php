...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $sql = "INSERT INTO `player_paths` (`session_id`, `x`, `y`, `z`, `position_time`, `player_id`) VALUES (?, ?, ?, ?, ?, ?)";
        if ($stmt = $conn->prepare($sql)) {
            // Bind the values from the JSON data to the SQL parameters
            $stmt->bind_param('ssssss', $data['sessionId'], $data['x'], $data['y'], $data['z'], $data['time'], $data['id']);
